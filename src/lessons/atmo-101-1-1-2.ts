...
**Water vapour: the powerful amplifier.** Water vapour (H₂O) is the most abundant greenhouse gas and the dominant absorber of infrared radiation in the lower troposphere. However, it is controlled by temperature: warmer air can hold more water vapour (following the Clausius-Clapeyron relationship, roughly 7% more per degree of warming). This means water vapour is a **feedback**, not a forcing: it amplifies warming or cooling caused by other agents (like CO₂) but cannot itself initiate climate change — if temperatures return to their original level, water vapour would condense out. The water vapour feedback roughly doubles the warming caused by CO₂ alone. Water vapour also transports energy: when ocean water evaporates, it absorbs latent heat; when it condenses in clouds and rain, it releases this heat, which drives tropical convection and powers hurricanes.
...
**Ozone: two stories at two altitudes.** Just as chlorine is essential in a dilute swimming pool disinfectant but lethal if you inhale concentrated chlorine gas, the same O₃ molecule plays opposite roles depending on where it is. In the stratosphere, ozone absorbs UV-B (280–315 nm) and UV-C (100–280 nm), preventing these wavelengths from reaching the surface where they would cause DNA damage, skin cancer, and cataracts and disrupt photosynthesis. Stratospheric ozone is created by UV breaking O₂ apart (O₂ + UV → 2O; O + O₂ → O₃) and destroyed by catalytic cycles involving halogen radicals from CFCs. In the troposphere, ozone is formed secondarily when nitrogen oxides (NOₓ, from combustion) and volatile organic compounds (VOCs) react in sunlight — **photochemical smog**. Ground-level ozone irritates the respiratory system, damages crops, and reduces the growth of forests. The separation between "good" stratospheric ozone and "bad" tropospheric ozone is one of the clearest examples of why understanding altitude context is essential in atmospheric chemistry.`,
    ...
    },
    {
      type: 'concept',
      title: 'Key takeaways',
      body: `• **Water vapour amplifies but cannot initiate climate change**: Because it condenses out whenever temperatures drop, water vapour concentration is set by temperature — it is a feedback that roughly doubles CO₂-driven warming, but it cannot independently sustain elevated concentrations if the primary forcing (CO₂) is removed.
