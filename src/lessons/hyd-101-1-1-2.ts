  ...
  sections: [
    {
      ...
      body: `Precipitation is the primary input of freshwater to land surfaces. Three main mechanisms lift air to the dewpoint, triggering condensation and precipitation: convective (instability-driven; typical of tropical thunderstorms and afternoon showers), orographic (forced lifting over mountain barriers; produces 5,000+ mm/yr in windward ranges like the Western Ghats and Hawaiian Ko\'olau), and frontal/cyclonic (lifting along weather fronts and within extratropical cyclones). Global mean precipitation is about 990 mm/yr over land (compared to 1,020 mm/yr over ocean), but with extreme variability: the Atacama Desert receives < 0.1 mm/yr while Cherrapunji, India received 26,470 mm in a single year (1861).

Evapotranspiration (ET) is the combined flux of water vapour from land surfaces — including direct evaporation from soil, open water, and wet canopies, plus transpiration through plant stomata. ET accounts for ~73,000 km³/yr globally — about 65% of all land precipitation returns directly to the atmosphere this way. The Penman-Monteith equation (1948, adopted as FAO standard in 1998) calculates reference ET (ET₀) from meteorological variables: net radiation (Rn), air temperature (T), wind speed (u₂), and vapour pressure deficit (VPD). In humid climates ET is energy-limited; in arid climates it is water-limited. The Budyko framework describes this transition using the aridity index (PET/P).

Potential evapotranspiration (PET) is the ET that would occur with unlimited water supply — constrained only by available energy. Actual ET (AET) is the real flux, limited by available water. The ratio AET/PET (called the evaporative fraction or Budyko ratio) approaches 1.0 in humid environments and near 0 in hyper-arid ones. Globally, the mean AET/PET ≈ 0.65. Quantifying AET is critical for agricultural water management: the FAO Penman-Monteith method multiplies ET₀ by a crop coefficient (Kc) specific to crop type and growth stage to estimate crop water demand, forming the basis for irrigation scheduling worldwide.
