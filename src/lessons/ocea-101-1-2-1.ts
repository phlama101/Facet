...
**Biological cycling.** Seawater composition is not just controlled by inorganic chemistry — life plays a dominant role. **Nutrients** (nitrate, phosphate, silicate, iron) are essential for photosynthesis and are nearly depleted in the sunlit surface layer, where phytoplankton absorb them and incorporate them into biomass. When organisms die and sink, they decompose, releasing nutrients back into deep water in a process called the **biological pump**. The distribution of nutrients in the ocean is largely a biological phenomenon: surface waters are stripped of nutrients by photosynthesis; deep waters are enriched by remineralisation. Upwelling zones, where deep nutrient-rich water rises to the surface, are among the most productive ocean environments on Earth.`,
    ...
    },
    {
      ...
      questions: [
        ...
        {
          q: 'Sodium has a residence time of approximately 68 million years in the ocean, while aluminium has a residence time of only about 100 years. What does this difference in residence time reveal about how each element behaves chemically in seawater?',
          a: [
            'Sodium stays in the ocean longer because it is produced continuously by submarine volcanoes; aluminium is short-lived because it is rapidly evaporated from the ocean surface and lost to the atmosphere',
            'Sodium has a long residence time because it forms highly soluble ions that have no efficient removal mechanism in the open ocean; aluminium has a short residence time because it rapidly adsorbs onto sinking particles and is removed to the seafloor, so its concentration varies enormously by location and depth',
            'Sodium is heavier than aluminium and sinks to the deep ocean where it is preserved; aluminium is lighter and remains at the surface where it is rapidly photodegraded by ultraviolet sunlight',
            'The difference in residence times reflects the age of each element: sodium was dissolved in the ocean during early Earth history when rocks were first weathered; aluminium entered the ocean recently from industrial pollution and has not yet reached equilibrium',
          ],
          correct: 1,
          explain: 'Submarine volcanic production of sodium (A) is incorrect — sodium is primarily delivered to the ocean by rivers weathering silicate and evaporite minerals; hydrothermal systems do influence sodium to a degree but it is not the dominant source; and aluminium is certainly not evaporated from the ocean surface. Density-based sinking (C) does not apply to dissolved ions — aluminium in seawater is a dissolved species, not a particle, so it does not sink under gravity based on its atomic mass. Industrial pollution timing (D) is incorrect — aluminium has been delivered to the ocean by continental weathering for billions of years; the short residence time is a consequence of its chemistry, not its recent addition. The correct answer is B: residence time is determined by the balance between input and removal rates; sodium (Na⁺) is a conservative ion — it forms no insoluble compounds in seawater, is not taken up biologically in significant quantities, and is not adsorbed onto mineral surfaces; the only removal pathways are burial in evaporite deposits and hydrothermal exchange, both of which are slow; thus sodium accumulates to high concentrations and has a ~68 million year residence time, contributing to the principle of constant proportions; aluminium (Al³⁺), by contrast, is highly reactive — at seawater pH it forms hydroxide and oxide complexes that readily adsorb onto organic and mineral particles sinking through the water column; this scavenging removes aluminium from the water within about 100 years; as a result, aluminium concentrations vary by orders of magnitude between surface and deep water, and between ocean basins, making it a tracer of particle dynamics rather than a stable component of seawater chemistry.',
        },
      ],
    },
  ],
