...
**Annotated features.** Beyond lithology and contacts, a stratigraphic column records the sedimentary structures present at each horizon, because structures are the most direct indicators of depositional process. Cross-bedding (chevron symbols inclined in the dip direction) records migrating bedforms under unidirectional flow; the dip direction gives paleoflow. Ripple lamination records lower-velocity flow or wave oscillation. Hummocky cross-stratification (dome-shaped symbols) records storm-wave action below fair-weather wave base. Parallel lamination records high-velocity plane-bed flow or settling from suspension. Bioturbation symbols (spirals for Skolithos pipe burrows, asterisks for star-shaped traces) record the activity of burrowing organisms — a proxy for water oxygenation and sedimentation rate. Graded bedding (a triangle widening downward, representing grains getting coarser at the base) is the hallmark of turbidites. Mudcracks (inverted V symbols) record subaerial exposure and desiccation. Fossils — marine shells, plant material, vertebrate fragments — are marked at their occurrence horizons and provide direct biological context.`,
      cards: [
        ...
        },
        {
          ...
          examples: "Gradational: fine sand → silt → shale over 2 m = progressive offshore deepening · Sharp: shale directly on limestone with no transition = abrupt flooding event or brief exposure · Erosional: scoured base with limestone rip-ups in overlying sandstone = river channel cutting into limestone floodplain",
        },
        {
          ...
          examples: "Bioturbation index: BI 0 = no burrows (anoxic or rapid deposition) → BI 6 = completely homogenised (slow deposition, oxygenated) · Paleoflow from cross-bedding dip direction: consistent NE dip across 10 km = ancient river flowing NE · Mudcracks above limestone = lake or tidal flat exposure between carbonate deposition events",
        },
      ],
    },
    {
...
**Correlation between sections.** The real power of a stratigraphic column is unlocked when two or more sections from different locations are compared. Correlation — establishing which intervals in different sections are the same age — requires distinctive tie points. The most reliable is a **volcanic ash layer** (tuff): ash from a single eruption settles globally within days to years and is therefore the same age everywhere. If the same distinctive ash chemistry appears in two sections 50 km apart, every bed above the ash in one section is correlative with the beds above the ash in the other. The difference in height of the ash within each section reflects differences in local sedimentation rate: if the ash is 10 metres above the base in section A and 5 metres above the base in section B, section A accumulated twice as much sediment in the same pre-ash time interval — either a higher sedimentation rate or a greater accommodation.
      ...
      questions: [
        ...
        },
        {
          ...
          a: [
            ...
            "The ash layer is an isochron — it was deposited globally at the same instant from a single eruption, so every bed below the ash in both sections is older than every bed above it; this correlation ties the two sections to a common time datum; the height difference (15 m vs. 8 m) indicates that section A accumulated nearly twice as much sediment as section B in the same pre-ash time interval — section A had a higher sedimentation rate, greater accommodation, or was in a more proximal, higher-energy setting",
            "The height difference proves that section A was tilted by faulting after deposition — in the original horizontal state, the ash would have been at the same height in both sections; the 7 m discrepancy is a structural offset",
            "The ash layer is useful only if the two sections are within 1 km of each other; at 50 km, wind dispersal would have produced two ash layers of different ages at different elevations, and they cannot be correlated",
          ],
          correct: 1,
          explain: "Volcanic ash layers are among the most powerful stratigraphic tools available because a single explosive eruption deposits ash globally within days to years — geologically instantaneous. The ash is therefore a true time-marker (isochron): every rock below the ash in any section predates the eruption; every rock above it postdates the eruption. This allows perfect time correlation between sections regardless of how different the sediment looks above and below the ash in each section. The height difference does not mean the ash is at different ages — it means different amounts of sediment accumulated before the ash in each location. Section A has 15 m of pre-ash sediment; section B has only 8 m. If both started at the same time (same base), section A accumulated 15/8 ≈ 1.9 times more sediment before the eruption — a higher local sedimentation rate, deeper accommodation, or a more proximal (closer to source) position. This information is itself geologically valuable: it constrains the geometry of the basin and the distribution of depositional environments at the time. Option A is wrong — the same eruption absolutely can appear at different heights in different sections (that is the point); the heights record local sedimentation rates, not different eruptions. Option C misinterprets sedimentary thickness variation as tectonic offset — structural tilt would require the ash to be physically inclined, not simply at different elevations in two vertical sections. Option D is wrong — volcanic ash from large eruptions is routinely correlated across thousands of kilometres; some major tephras are identified on multiple continents.",
        },
      ],
    },
  ],
