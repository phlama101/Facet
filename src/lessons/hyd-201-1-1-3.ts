      ...
      cards: [
        {
          ...
          examples: 'Steep mountain stream (n=0.04, R=0.5m, S=0.01): V = 3.5 m/s · Lowland river (n=0.030, R=2m, S=0.0003): V = 1.4 m/s · Urban concrete channel (n=0.013, R=1m, S=0.005): V = 8.2 m/s · Mississippi at St Louis: Manning analysis gives Q ≈ 16,000 m³/s at flood stage',
        },
        {
        ...
        },
        {
          ...
          examples: 'Gravel-bed reach, bankfull Q = 50 m³/s: expected W ≈ 27m, d ≈ 1.4m · Mississippi headwaters to delta: W increases from 30m to 1,000m; d from 1m to 12m · Post-dam downstream: channel narrows 30–60% below major dams (sediment starvation) · Urban stream: impervious cover increase → Q^2 increases → channel incision and widening',
        },
        {
          ...
          examples: 'Mountain stream (V=4 m/s, d=0.5m): Fr = 4/√(9.81×0.5) = 1.81 (supercritical) · Lowland river (V=1.5 m/s, d=3m): Fr = 1.5/√(9.81×3) = 0.28 (subcritical) · Hydraulic jump on spillway: Fr1=5 → sequent depth ratio d2/d1 = 0.5(√(1+8Fr²)−1) ≈ 6.6 · Oroville Dam chute: supercritical flow Fr ≈ 3–4 caused destructive cavitation',
        },
      ],
    },
    {
...
**2D hydraulic modelling** with HEC-RAS 2D and LISFLOOD-FP has transformed floodplain mapping since the 2000s. LiDAR-derived digital elevation models (0.5–2 m resolution, vertical accuracy 10–15 cm) provide the terrain on which the 2D diffusion or dynamic wave equations are solved on structured or unstructured grids. Flow spreads naturally across floodplains, around buildings, and through bridge openings without needing to pre-specify flow paths. The 2017 Oroville Dam crisis (California) demonstrated the value of this capability: CDFW and USACE ran HEC-RAS 2D in near-real time to assess downstream inundation scenarios for the 188,000-person evacuation zone below the dam.`,
      cards: [
        ...
        },
        {
          ...
          examples: 'Hoover Dam: 35 km³ storage; 100-yr inflow peak 2,300 m³/s → attenuated to 1,100 m³/s · Small detention pond (0.01 km²): can reduce 10-yr peak by 40% for 100 ha urban catchment · Oroville reservoir (4.4 km³): incoming Feb 2017 peak ≈ 1,600 m³/s; dam held but spillway failed · Table lookup: pre-computed (2S/Δt + O) vs O curve for each routing step',
        },
        {
          name: 'Oroville Dam 2017: HEC-RAS 2D in Emergency Mode',
          icon: Globe,
          color: BRAND.coral,
          desc: 'February 2017: 155 mm rainfall in 7 days (record); Lake Oroville inflows peaked at 4,600 m³/s. Main spillway concrete chute failed on Feb 7, exposing compressible fill underneath — 14 m deep erosion chasm formed. Emergency spillway used for first time: unlined hillside began eroding. Evacuation of 188,000 ordered Feb 12. HEC-RAS 2D simulated dam-breach and emergency spillway failure scenarios for Feather River valley using 1m LiDAR DEM. Inundation depth maps guided evacuation zone boundaries.',
          examples: 'Without evacuation models: downstream Oroville, Gridley, Marysville — population 188,000 at risk · Dam did NOT fail; controlled releases managed crisis · Post-crisis: $1.1 billion spillway reconstruction (2018–2019) · Independent Forensic Team report: unlined emergency spillway was permitted without adequate geotechnical analysis · USACE HEC-RAS 2D: 10m unstructured mesh; 9 breach scenarios simulated in <4 hours',
        },
        {
          name: 'FEMA Floodplain Mapping: 100-Year Flood',
          icon: Droplets,
          color: BRAND.gold,
          desc: '100-year flood = 1% annual exceedance probability (AEP) flood. FEMA National Flood Insurance Program (NFIP) maps Special Flood Hazard Area (SFHA) — land with 1% AEP flood. Zone A: approximate; Zone AE: detailed study with BFE (base flood elevation); Zone X: outside 500-yr floodplain. Mapping method: HEC-HMS generates design hydrograph → HEC-RAS steady-flow profiles → floodplain boundary digitised on aerial imagery. LiDAR-based remapping (Cooperating Technical Partners) improving accuracy from ±1m to ±15cm vertical. 37 million US structures in SFHA; $1.3 trillion at risk.',
      ...
      interaction: {
        ...
        nodes: [
          ...
          },
          {
            ...
            description: 'One-dimensional hydraulic models solve the Saint-Venant equations (or steady backwater equations for design applications) along the channel thalweg, with cross-sections sampled at regular intervals (50–500 m). Water surface profiles are computed iteratively from downstream (subcritical) or upstream (supercritical) boundary conditions. Results include water surface elevation, velocity, and shear stress at each cross-section. Standard engineering application: compute 100-year flood water surface profile for FEMA flood insurance rate mapping. HEC-RAS 1D is the FEMA standard method for detailed flood insurance studies.',
            color: BRAND.coral,
          },
          {
            id: 'hec-ras-2d',
            label: 'HEC-RAS 2D / LISFLOOD-FP Inundation',
            description: 'Two-dimensional hydraulic models solve the 2D diffusion or dynamic wave equations on a fine computational mesh covering the floodplain. Water flows freely in two dimensions around buildings, through road culverts, and over levees. Required inputs: high-resolution terrain model (LiDAR DEM, 0.5–2 m resolution), roughness coefficients for floodplain land cover, and the 1D boundary conditions from channel routing. Output: depth and velocity grids covering the entire floodplain, updated at each time step (typically 1–60 seconds for explicit solvers). Used for: FEMA remapping, dam-break analysis, real-time emergency evacuation mapping (Oroville 2017), property-level flood risk assessment.',
            color: BRAND.ruby,
          },
          {
            id: 'flood-map',
      ...
      questions: [
        ...
        },
        {
          ...
          a: [
            ...
            '2D hydraulic models replace LiDAR terrain data with satellite radar altimetry (SRTM at 30 m resolution), which provides sufficient accuracy for floodplain mapping in all environments including heavily vegetated tropical rivers; they require no field data because all roughness values are derived from satellite multispectral imagery using machine learning algorithms',
