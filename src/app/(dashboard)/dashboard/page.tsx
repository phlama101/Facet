...
export default async function DashboardPage() {
  ...
  const nextLesson = LESSON_LIST.find(l => !completed.includes(l.id)) ?? LESSON_LIST[0]
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard label="Lessons"  value={completed.length}    sub={`/ ${LESSON_LIST.length}`} icon={BookOpen} accent={BRAND.jade} />
      ...
      >
        ...
        <div className="grid gap-2">
          {LESSON_LIST.slice(0, 4).map(lesson => {
